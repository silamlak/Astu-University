...
const ApplicationDetail = ({ d }) => {
  const navigate = useNavigate();

  const handleStatusChange = async (status) => {
    try {
      const res = await axios.put(`${API}/college//application/status/${d._id}`, {
        ...d,
        college_status: status,
      });
      console.log(res.data) // Redirect to the list or another appropriate page
    } catch (error) {
