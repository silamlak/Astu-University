...
const StudentDetail = ({ d, sIs }) => {
  ...
  const handleStudentStatus = async () => {
    ...
    try {
      await axios.put(`${API}/college/student/status`, {file}, {
        headers: {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-600 bg-opacity-75 flex justify-center items-center p-6">
      <div className="container mx-auto bg-white dark:bg-gray-800 max-h-[600px] overflow-y-auto shadow-lg rounded-lg relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6 p-6 max-h-[60vh] overflow-y-auto">
          ...
          <div>
            <p className="text-gray-600 dark:text-gray-100">Learning Status</p>
            <p className={`text-sm uppercase ${getStatusColor(d.status)}`}>
              {d.status}
            </p>
          </div>
          <div>
            ...
            {d.duration?.previousDurations && viewHistory && (
              <div className="flex flex-col gap-2 border border-gray-600 w-fit p-2 mt-4">
                {d.duration.previousDurations?.map((history, index) => (
                  <div>
                    <div key={history._id} className="flex gap-2 items-center">
                      <div>
                        <p className="text-sm text-gray-600 dark:text-gray-300">
                          {new Date(history.from).toLocaleDateString()}
                        </p>
                      </div>
                      <FaArrowRightLong className="text-sm text-gray-600 dark:text-gray-300" />
                      <div>
                        <p className="text-sm text-gray-600 dark:text-gray-300">
                    ...
                    <button
                      className=" text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-800"
                      onClick={() => viewFile(d.duration.update_file[index])}
                    >
                      View File
                    </button>
                  </div>
          ...
          <div className="flex gap-2 p-4">
            <div className="relative">
              ...
              >
                ...
                <option value="Finished Dismissed">Finished Dismissed</option>
              </select>
